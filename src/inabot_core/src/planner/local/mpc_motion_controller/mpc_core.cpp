...
public:
  ...
  double _w_cte, _w_etheta, _w_vel, _w_angvel, _w_accel, _w_angvel_d,
      _w_accel_d;
  ...
  void LoadParams(const std::map<string, double> &params) {
    ...
    _ref_cte = params.find("REF_CTE") != params.end() ? params.at("REF_CTE")
                                                      : _ref_cte;
    _ref_etheta = params.find("REF_ETHETA") != params.end()
                      ? params.at("REF_ETHETA")
                      : _ref_etheta;
    _ref_vel =
        params.find("REF_V") != params.end() ? params.at("REF_V") : _ref_vel;

    _w_cte = params.find("W_CTE") != params.end() ? params.at("W_CTE") : _w_cte;
    _w_etheta =
        params.find("W_EPSI") != params.end() ? params.at("W_EPSI") : _w_etheta;
    ...
    _w_angvel_d = params.find("W_DANGVEL") != params.end()
                      ...
                      : _w_angvel_d;
    _w_accel_d =
        params.find("W_DA") != params.end() ? params.at("W_DA") : _w_accel_d;
  ...
  void operator()(ADvector &fg, const ADvector &vars) {
    ...
    for (int i = 0; i < _mpc_steps; i++) {
      fg[0] += _w_cte * CppAD::pow(vars[_cte_start + i] - _ref_cte,
                                   2); // cross deviation error
      fg[0] += _w_etheta * CppAD::pow(vars[_etheta_start + i] - _ref_etheta,
                                      2); // heading error
      fg[0] +=
          _w_vel * CppAD::pow(vars[_v_start + i] - _ref_vel, 2); // speed error

      cost_cte += _w_cte * CppAD::pow(vars[_cte_start + i] - _ref_cte, 2);
      cost_etheta +=
          (_w_etheta * CppAD::pow(vars[_etheta_start + i] - _ref_etheta, 2));
      cost_vel += (_w_vel * CppAD::pow(vars[_v_start + i] - _ref_vel, 2));
    }
    cout << "-----------------------------------------------" << endl;
    cout << "cost_cte, etheta, velocity: " << cost_cte << ", " << cost_etheta
         << ", " << cost_vel << endl;
    ...
    for (int i = 0; i < _mpc_steps - 2; i++) {
      fg[0] +=
          _w_angvel_d *
          CppAD::pow(vars[_angvel_start + i + 1] - vars[_angvel_start + i], 2);
      fg[0] += _w_accel_d *
               CppAD::pow(vars[_a_start + i + 1] - vars[_a_start + i], 2);
    }
    ...
    for (int i = 0; i < _mpc_steps - 1; i++) {
      ...
      // AD<double> trj_grad0 = CppAD::atan(coeffs[1] + 2 * coeffs[2] * x0 + 3 *
      ...
      fg[2 + _cte_start + i] =
          cte1 - ((f0 - y0) + (v0 * CppAD::sin(etheta0) * _dt));
      // fg[2 + _etheta_start + i] = etheta1 - ((theta0 - trj_grad0) + w0 *
      // _dt);//theta0-trj_grad0)->etheta : it can have more curvature
      // prediction, but its gradient can be only adjust positive plan.
...
// ====================================
// MPCCore class definition implementation.
// ====================================
MPCCore::MPCCore() {
  // Set default value
  _mpc_steps = 20;
...
void MPCCore::LoadParams(const std::map<string, double> &params) {
  _params = params;
  ...
  _max_angvel = _params.find("ANGVEL") != _params.end() ? _params.at("ANGVEL")
                                                        : _max_angvel;
  _max_throttle = _params.find("MAXTHR") != _params.end() ? _params.at("MAXTHR")
                                                          : _max_throttle;
