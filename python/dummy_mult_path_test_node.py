...
class DummyOdomAndPathNode(Node):
    def __init__(self):
        ...
        self.path_pub = self.create_publisher(Path, 'fms_path', 10) # fms_path  planned_path
    ...
    def add_initial_goal(self):
        with self.lock:
            ...
            for i in range(3):  # 3개의 목표점 생성
                goal = self.generate_next_goal(distance=0.4 * (i+1))
            ...
            self.get_logger().info(
                "초기 목표점 3개 생성: " +
                ", ".join([f"({p.pose.position.x:.2f},{p.pose.position.y:.2f})" for p in self.path_msg.poses])
            )
    ...
    def goal_reached_callback(self, msg):
        if msg.data:
            with self.lock:
                self.path_msg.poses.clear()
                for i in range(3):  # 새로운 3개의 목표점 생성
                    goal = self.generate_next_goal(distance=0.4 * (i+1))
                    pose_stamped = PoseStamped()
                    pose_stamped.header.frame_id = 'odom'
                    pose_stamped.header.stamp = self.get_clock().now().to_msg()
